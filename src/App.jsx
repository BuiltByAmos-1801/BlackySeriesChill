...
import instaImage from "./images/68f26c7a06e9f_download.jpg.jpeg";
...
const heroLines = ["Original lyrics", "Live broadcast", "Photography", "Collaboration ready"];
const liveStats = [
    ...
    <section id="home" className="hero">
      <div className="hero-grid">
        <div className="hero-copy">
          ...
          <p className="hero-text">The official home of Sadiq Khan's music, lyrics, visuals, and collaboration work.</p>
      ...
      <div className="music-grid">
        ...
        <div className="release-stack">
          {releases.map((release) => (
            <article className="release-card glass-card" key={release.title}>
    ...
    <Section id="socials" eyebrow="Social Hub" title="Follow The Signal Everywhere">
      <div className="social-grid">
        {socials.map(({ name, text, href, icon: Icon }) => (
          <a className="social-card glass-card" href={href} target="_blank" rel="noreferrer" key={name} aria-label={`Visit Blacky Series Chill on ${name}`}>
...
function Collaboration() {
  return (
    <section className="collab glass-card">
      <Mic2 size={38} />
  ...
  return (
    <Section id="faq" eyebrow="FAQ" title="Frequently Asked Questions">
      <div className="faq-grid">
        {faqItems.map((item) => (
          <article className="faq-card glass-card" key={item.question}>
            <h3>{item.question}</h3>
            <p>{item.answer}</p>
          </article>
        ))}
...
function Footer() {
  return (
    <footer className="footer">
      ...
      <a className="to-top" href="#home" aria-label="Back to top"><ArrowUp size={20} /></a>
    </footer>
  );
}
